...
async function startServer() {
  try {
    ...
    // 1. API ROUTES (Must be before any SPA/Vite middleware)
    ...
    app.get('/api/flights/search', async (req, res) => {
      ...
      try {
        ...
        if (returnDate) params.append('returnDate', returnDate as string);
        ...
        console.error('[API] Flight API Error:', errorMessage);
        res.status(500).json({ error: 'Failed to fetch live flight offers', details: errorMessage });
      }
    });
    ...
    app.get('/api/hotels/search', async (req, res) => {
      ...
      try {
        ...
        const params = new URLSearchParams({ hotelIds, adults: (adults as string) || '1', currency: 'USD' });
        ...
        console.error('[API] Hotel API Error:', error.message);
    ...
    app.get('/api/activities/search', async (req, res) => {
      ...
      try {
        ...
        console.error('[API] Activities API Error:', error.message);
    ...
    app.post('/api/itinerary/confirm', async (req, res) => {
      ...
      if (!email || !itinerary) return res.status(400).json({ error: 'Email and itinerary are required' });

      try {
        await new Promise(resolve => setTimeout(resolve, 1000));
        res.json({ success: true, message: 'Itinerary successfully sent.' });
      } catch (error: any) {
