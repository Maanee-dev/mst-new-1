...
const ResortSlide: React.FC<{ 
  ...
  isVisible: boolean;
}> = ({ resort, isLiked, isInBag, toggleLike, addItem, handleShare, openComments, isVisible }) => {
  const [activeIdx, setActiveIdx] = useState(0);
  const [commentCount, setCommentCount] = useState(0);
  const galleryRef = useRef<HTMLDivElement>(null);
  // Limit images on mobile to prevent memory issues
  const isMobile = typeof window !== 'undefined' && window.innerWidth < 768;
  const images = (resort.images || []).slice(0, isMobile ? 3 : 8);
      ...
      >
        {images.map((img, idx) => (
          <div key={idx} className="w-full h-full flex-shrink-0 snap-center relative">
            {/* Only render image if slide is visible or it's the first image */}
            {(isVisible || idx === 0) && (
              <img 
                src={img} 
                alt={`${resort.name} ${idx}`} 
...
const DiscoveryFeed: React.FC = () => {
  ...
  useEffect(() => {
    const fetchResorts = async () => {
      setLoading(true);
      try {
        const { data, error } = await supabase.from('resorts').select('*').limit(10); // Limit to 10 for performance
        if (error) throw error;
        if (data && data.length > 0) {
          setResorts(data.map(mapResort));
        } else {
          setResorts(RESORTS.slice(0, 5)); // Reduced fallback limit
        }
      } catch (err) {
  ...
  const handleScroll = () => {
    if (scrollRef.current) {
      ...
      // Keep track of visible indices (current, prev, next) to optimize rendering
      const newVisible = new Set([index]);
      if (index > 0) newVisible.add(index - 1);
      if (index < resorts.length - 1) newVisible.add(index + 1);
  ...
  return (
    <AnimatePresence>
      ...
      >
        {/* Persistent Top Bar */}
        <div className="absolute top-0 left-0 right-0 z-[505] p-6 md:p-10 flex justify-between items-start pointer-events-none">
          <button 
          ...
          <div className="flex flex-col items-center pointer-events-auto">
             <svg viewBox="0 0 600 600" className="w-20 h-20 md:w-28 md:h-28 -my-6 md:-my-10 fill-white opacity-90 drop-shadow-2xl">
               <g transform="translate(0.000000,600.000000) scale(0.100000,-0.100000)">
                 ...
                 <path d="M3146 2774 c-10 -38 10 -55 24 -20 7 18 16 26 28 24 -5 31 -147 7 -160 -27 -16 -16 -28 25 -28 38 0 52 12 30 25 -5 3 -10 42 -10 86 0 88 12 101 49 52 l21 -28 -2 35 -3 35 -81 3 c-81 3 -82 3 -88 -24z"/>
          ...
          <button 
            onClick={() => navigate(-1)}
            className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center bg-black/20 hover:bg-black/40 backdrop-blur-xl rounded-full text-white transition-all group border border-white/10 pointer-events-auto"
